...
CRedBullTex::CRedBullTex(LPDIRECT3DDEVICE9 pDevice)
:CGameObject(pDevice)
...
CRedBullTex::~CRedBullTex()
{
	Release();
}
HRESULT CRedBullTex::Initialize(void)
{
	...
	m_pResourceMgr->GetVtxInfo(ENGINE::RESOURCE_STATIC, L"Buffer_RcTex", m_pVertex);

	FAILED_CHECK(AddComponent());

	return S_OK;
}
void CRedBullTex::Update(void)
{
...
CRedBullTex* CRedBullTex::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CRedBullTex* pMonster = new CRedBullTex(pDevice);
...
HRESULT CRedBullTex::AddComponent(void)
{
	...
	// Texture
	pComponent = m_pResourceMgr->CloneResource(ENGINE::RESOURCE_DYNAMIC, L"Texture_RedBull");
...
void CRedBullTex::SetTransForm(void)
{
...
void CRedBullTex::SetMonsterTex(D3DXVECTOR3* vPos)
{
	m_pPos = vPos;
}
