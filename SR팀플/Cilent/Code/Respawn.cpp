...
#include "ObjectCol.h"
#include "KeyMgr.h"
#include "RespawnTex.h"

CRespawn::CRespawn(LPDIRECT3DDEVICE9 pDevice)
: CGameObject(pDevice)
...
HRESULT CRespawn::Initialize(void)
{
	...
	m_pMonsterTex = CRespawnTex::Create(m_pDevice);
...
HRESULT CRespawn::AddComponent(void)
{
...
CRespawn* CRespawn::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CRespawn* pPlayer = new CRespawn(pDevice);
...
void CRespawn::SetTransForm(void)
{
...
void CRespawn::SetPos(const D3DXVECTOR3& vPos)
{
	m_pInfo->m_vPos = vPos;
}

D3DXVECTOR3 CRespawn::GetMaxPos(const ENGINE::VTXCUBE* pCube)
{
	D3DXVECTOR3	vMax;
...
D3DXVECTOR3 CRespawn::GetMinPos(const ENGINE::VTXCUBE* pCube)
{
	D3DXVECTOR3	vMin;
...
const	ENGINE::VTXCUBE* CRespawn::GetCubeVertex()
{
	return m_pColVtx;
}
D3DXVECTOR3* CRespawn::GetPos(void)
{
