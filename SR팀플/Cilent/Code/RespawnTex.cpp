...
CRespawnTex::CRespawnTex(LPDIRECT3DDEVICE9 pDevice)
:CGameObject(pDevice)
...
CRespawnTex::~CRespawnTex()
{
	Release();
}
HRESULT CRespawnTex::Initialize(void)
{
...
CRespawnTex* CRespawnTex::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CRespawnTex* pMonster = new CRespawnTex(pDevice);
...
HRESULT CRespawnTex::AddComponent(void)
{
	...
	// Texture
	pComponent = m_pResourceMgr->CloneResource(ENGINE::RESOURCE_STATIC, L"Texture_Respawn");
...
void CRespawnTex::SetTransForm(void)
{
...
void CRespawnTex::SetMonsterTex(D3DXVECTOR3* vPos)
{
	m_pPos = vPos;
}
