...
CStage1::CStage1(LPDIRECT3DDEVICE9 pDevice)
...
CStage1* CStage1::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CStage1* pStage = new CStage1(pDevice);
...
HRESULT CStage1::Initialize(void)
{
	...
	CSoundMgr::GetInstance()->PlayBgm(L"run_bgm2.wav");
...
void CStage1::Update(void)
{
	...
	if(bLevel1)
	{
		...
		m_pManagement->SceneChange(CSceneSelector(SC_SHOT));
		return;
	}
	if(m_bInitHp == false)
	{
	...
	if(GetAsyncKeyState(VK_F2) /*&& m_iImageCnt == 287*/)
	{
		iScore += int(fHp * 10.f);
		CSoundMgr::GetInstance()->StopBga();
		m_pManagement->SceneChange(CSceneSelector(SC_SHOT));
...
HRESULT CStage1::Add_Environment_Layer(void)
{
	return S_OK;
}

HRESULT CStage1::Add_GameLogic_Layer(void)
{
	...
	ChangeTerrain((CTerrain*)pGameObject, L"Terrain_Stage1");
	...
	((CPlayer*)pGameObject)->SetStageState(CPlayer::ST_STAGE1);
	...
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
	((CHotSix*)pGameObject)->SetPos(D3DXVECTOR3(17.f, 9.f, 64.f));
	pLayer->AddObject(L"HotSix", pGameObject);

	// RedBull
	pGameObject = CRedBull::Create(m_pDevice);
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
	((CRedBull*)pGameObject)->SetPos(D3DXVECTOR3(11.f, 1.f, 106.f));
	pLayer->AddObject(L"RedBull", pGameObject);

	//Respawn
	pGameObject = CRespawn::Create(m_pDevice);
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
	((CRespawn*)pGameObject)->SetPos(D3DXVECTOR3(5.f, 2.f, 10.f));
	pLayer->AddObject(L"Respawn", pGameObject);

	pGameObject = CRespawn::Create(m_pDevice);
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
	((CRespawn*)pGameObject)->SetPos(D3DXVECTOR3(14.f, 2.f, 83.f));
	pLayer->AddObject(L"Respawn", pGameObject);

	pGameObject = CRespawn::Create(m_pDevice);
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
	((CRespawn*)pGameObject)->SetPos(D3DXVECTOR3(14.f, 2.f, 227.f));
	pLayer->AddObject(L"Respawn", pGameObject);
...
HRESULT CStage1::Add_UI_Layer(void)
{
	ENGINE::CLayer*		pLayer = ENGINE::CLayer::Create(m_pDevice);
	NULL_CHECK_RETURN(pLayer, E_FAIL);
...
void CStage1::Load(ENGINE::CLayer* pLayer)
{
	...
	fopen_s(&pFile, "../../Data/Stage1.dat", "rb");
...
void CStage1::ChangeTexture(CObstacle* pCubeObj, const wstring& wstrName)
{
	if(pCubeObj == NULL)
		return;
...
void CStage1::ChangeTerrain(CTerrain* pCubeObj, const wstring& wstrName)
{
	if(pCubeObj == NULL)
		return;
...
void CStage1::ChangeBack(CBack* pCubeObj, const wstring& wstrName)
{
	if(pCubeObj == NULL)
		return;
...
HRESULT CStage1::CreateBack(ENGINE::CLayer* pLayer)
{
	ENGINE::CGameObject*		pGameObject = NULL;
	...
	ChangeBack((CBack*)pGameObject, L"Terrain_Stage3Back");
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
