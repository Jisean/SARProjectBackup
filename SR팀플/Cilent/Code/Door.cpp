...
HRESULT CDoor::Initialize(void)
{
	...
	m_pMonsterTex = CDoorTex::Create(m_pDevice);
...
{
	CDoor* pPlayer = new CDoor(pDevice);
...
void CDoor::SetPos(const D3DXVECTOR3& vPos)
{
	m_pInfo->m_vPos = vPos;
}

D3DXVECTOR3 CDoor::GetMaxPos(const ENGINE::VTXCUBE* pCube)
{
	D3DXVECTOR3	vMax;
...
D3DXVECTOR3 CDoor::GetMinPos(const ENGINE::VTXCUBE* pCube)
{
	D3DXVECTOR3	vMin;
...
const	ENGINE::VTXCUBE* CDoor::GetCubeVertex()
{
	return m_pColVtx;
}
