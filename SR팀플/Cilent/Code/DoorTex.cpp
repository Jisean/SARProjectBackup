...
CDoorTex::CDoorTex(LPDIRECT3DDEVICE9 pDevice)
:CGameObject(pDevice)
...
HRESULT CDoorTex::Initialize(void)
{
...
void CDoorTex::Update(void)
{
	...
	m_pInfo->m_vPos.x = (*m_pPos).x;
	m_pInfo->m_vPos.y = (*m_pPos).y + 1;
	m_pInfo->m_vPos.z = (*m_pPos).z;
...
CDoorTex* CDoorTex::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CDoorTex* pMonster = new CDoorTex(pDevice);
...
HRESULT CDoorTex::AddComponent(void)
{
	...
	// Texture
	pComponent = m_pResourceMgr->CloneResource(ENGINE::RESOURCE_STATIC, L"Texture_Door");
...
void CDoorTex::SetTransForm(void)
{
...
void CDoorTex::SetMonsterTex(D3DXVECTOR3* vPos)
{
	m_pPos = vPos;
}
