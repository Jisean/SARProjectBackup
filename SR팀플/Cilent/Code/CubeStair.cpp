...
CCubeStair::CCubeStair(LPDIRECT3DDEVICE9 pDevice)
: CObstacle(pDevice)
...
CCubeStair::~CCubeStair()
{
	Release();
}


HRESULT CCubeStair::Initialize(void)
{
	SetTransForm();
...
HRESULT CCubeStair::AddComponent(void)
{
...
CCubeStair* CCubeStair::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CCubeStair* pCubeObj = new CCubeStair(pDevice);
...
void CCubeStair::SetTransForm(void)
{
...
void CCubeStair::SetPos(const D3DXVECTOR3& vPos)
{
	m_pInfo->m_vPos = vPos;
	m_vCopyPos = vPos;
}

const	ENGINE::VTXCUBE* CCubeStair::GetCubeVertex()
{
	return m_pColVtx;
}

D3DXVECTOR3* CCubeStair::GetPos(void)
{
	return &m_vCopyPos;
}
