...
void CSpinSnow::Release(void)
{
	...
	for(int i = 0; i < 5; ++i)
	{
		if(m_pSnowMan[i])
		{
			m_pInfoSubject->UnSubscribe(m_pSnowMan[i]->GetCamObserver());
			ENGINE::Safe_Delete(m_pSnowMan[i]);
		}

		if(m_pSnowMan_Body[i])
		{
			m_pInfoSubject->UnSubscribe(m_pSnowMan_Body[i]->GetCamObserver());
...
HRESULT CSpinSnow::AddComponent(void)
{
	...
	// Texture
	pComponent = m_pResourceMgr->CloneResource(ENGINE::RESOURCE_DYNAMIC, L"SnowFloor");
...
void CSpinSnow::SnowManCreate(void)
{
	...
	for(int i = 0; i < 5; ++i)
	{
		...
		m_pSnowMan_Body[i]->SetvScale(D3DXVECTOR3(1.8f, 1.8f, 1.8f));
	}
