...
CSpinblade::CSpinblade(LPDIRECT3DDEVICE9 pDevice)
: CObstacle(pDevice)
...
CSpinblade::~CSpinblade()
{
	Release();
}

HRESULT CSpinblade::Initialize(void)
{
...
void CSpinblade::Update(void)
{
	...
	m_fAngle -= D3DXToRadian(150.f) * ENGINE::CTimeMgr::GetInstance()->GetTime();
...
HRESULT CSpinblade::AddComponent(void)
{
...
CSpinblade* CSpinblade::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CSpinblade* pPlayer = new CSpinblade(pDevice);
...
void CSpinblade::SetTransForm(void)
{
...
void CSpinblade::SetPos(const D3DXVECTOR3& vPos)
{
	m_pInfo->m_vPos = vPos;
}


void CSpinblade::Move(void)
{
	D3DXVECTOR3		vDir;
...
D3DXVECTOR3 CSpinblade::GetMaxPos(const ENGINE::VTXCUBE* pCube)
{
	D3DXVECTOR3	vMax;
...
D3DXVECTOR3 CSpinblade::GetMinPos(const ENGINE::VTXCUBE* pCube)
{
	D3DXVECTOR3	vMin;
...
const	ENGINE::VTXCUBE* CSpinblade::GetCubeVertex()
{
	return m_pColVtx;
}
D3DXVECTOR3* CSpinblade::GetPos(void)
{
